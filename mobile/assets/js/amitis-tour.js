...
function showDescription(el, descClass) {
  ...
  const parentContainer = el.closest(".price-type");
  const descBox = parentContainer
    .closest(".hotel-card")
    .querySelector(`.${descClass}`);
  const svgIcon = el.querySelector("svg");
  ...
  document.querySelectorAll('[onclick^="showDescription"]').forEach((span) => {
    ...
    if (descClassMatch) {
      ...
      const parentContainer =
        span.closest(".price-type") || span.closest(".hotel-card");
...
const renderDescriptionHotelCost = async (element) => {
  try {
    ...
    console.error(
      "renderDescriptionHotelCost=" + err.lineNumber + "," + err.message
    );
    return "";
  }
};
...
function openTabInfo(element, tabId) {
  ...
  document
    .querySelectorAll(
      '[id^="service-tour"], [id^="tour-description"], [id^="required-documents-tour"]'
    )
    .forEach((tab) => {
...
document.addEventListener("DOMContentLoaded", function () {
  openPlan(
    document.querySelector("[onclick=\"openPlan(this , '1-day')\"]"),
    "1-day"
  );
});
function openPlan(element, target) {
  ...
  document.querySelectorAll(".plan-days").forEach((el) => {
    ...
    el.querySelector("figure").classList.remove(
      "!blur-none",
      "!w-[184px]",
      "!h-[184px]"
    );
  ...
  element
    ...
    .classList.add("!blur-none", "!w-[184px]", "!h-[184px]");
}
...
function toggleSocialMedia(element) {
  element
    .closest(".share-social-container")
    .querySelector(".drop-share")
...
<g transform="translate(0.000000,1280.000000) scale(0.100000,-0.100000)" stroke="none" fill="${
...
<g transform="translate(0.000000,910.000000) scale(0.100000,-0.100000)" fill="${
          document.querySelector(".layout__body__container").dataset
            .primarycolor
        }" stroke="none">
...
const renderRouteStop = async (element) => {
  try {
    if (element && element.info.stop && element.info.stop !== "") {
      return `<div class="mr-3 my-8 text-xs">${getStopLabel()}<span class="mr-1 ml-1">${
        element.info.stop
...
const renderTourDateModal = (element, className) => {
  try {
    document
      .querySelector(".tour__date__modal__container")
...
const renderHotels = async (element, type) => {
  try {
    if (!element || !element.hotelinfo || !element.hotelinfo[0].hotels)
      return "";
    ...
    for (let index = 0; index < hotels.length; index++) {
      ...
      const hotelImg =
        hotel.image && hotel.image.length > 0 ? hotel.image : fallbackImg;
      ...
      imageSection += `
        ...
        <img class="z-10 "
            ...
            <figure data-index="${index}" class="${
        index > 0 ? "-mr-24" : ""
      } bg-neutralcolor-400 border-l-8 border-neutralcolor-400 w-fit rounded-r-3xl rounded-l-[264px] relative ${imageOrder}">
                <img 
        ...
        data-basiscoreid="${hotel.basiscoreid}" 
                ...
                <figcaption class="group bg-primary text-white rounded-full w-fit h-6 absolute bottom-5 left-12 flex items-center gap-x-1 justify-center px-1">
                    <span class="text-xs font-yekanbakhsemiboldFA text-center hidden group-hover:inline-block line-clamp-1 text-nowrap ">${escapeHtml(
                      hotel.location
                    )}</span>
      ...
      textSection += `
                <div class="min-w-[48%] inline-block my-4 float-left">
                    <div class="h-12 bg-neutralcolor-50 rounded-[4px] w-fit flex flex-row-reverse items-center px-2 gap-x-3 float-left">
                        <div class="tourInventory__details__item__rate flex gap-0.5" data-value="${
                          hotel.star || 0
                        }">
                        ...
                        <span class="relative cursor-pointer group flex justify-center items-center">
                            <span class="text-xs font-yekanbakhregularFA text-txtneutral-900 underline uppercase 
                                      ...
                                      tourInventory__details__item__service flex justify-center items-center gap-x-1" data-value="${
                                        hotel.service.vid
                                      }">
                                ${await renderSummaryServiceHotel(
                                  hotel,
                                  element.booking
                                )}
                            </span>
    ...
    console.error(
      "renderHotels=" + (err.lineNumber || "-") + "," + err.message
    );
    return "";
  }
};
      ...
      document
        ...
        .forEach((img) => {
          ...
          if (img.dataset.id) {
            ...
            img.dataset.basiscoreid &&
            img.dataset.basiscoreid.trim() !== ""
          ) {
            const figure = img.closest("figure");
            if (figure) {
...
<path d="M2.80133 18.0844C3.00334 17.9222 3.25079 17.8594 3.50324 17.8594H20.5032C20.7557 17.8594 21.0031 17.9222 21.2052 18.0844C21.4075 18.2468 21.5092 18.4647 21.5535 18.6706C21.6361 19.0548 21.5406 19.4911 21.4256 19.8366C21.1035 20.8027 20.1981 21.4553 19.1794 21.4553H4.82711C3.80834 21.4553 2.90303 20.803 2.58099 19.8368C2.46599 19.4913 2.37035 19.0548 2.45296 18.6706C2.49723 18.4647 2.599 18.2468 2.80133 18.0844Z" fill="black"/>
      ...
      return `<span class="text-xs font-yekanbakhregular leading-5">${sevice}</span><span class="hidden">${await renderHotelBooking(
        booking
      )}</span>`;
    }
  } catch (err) {
...
const onsubmitTourForm = async (element, event) => {
  try {
    ...
    fetch(`${element.getAttribute("action")}`, {
      ...
      body: `usedforid=${element.querySelector(".usedforid").value}&mid=${
        element.querySelector(".mid").value
      }&title=${element.querySelector(".title").value}&comment=${
        element.querySelector(".comment").value
      }&captchaid=${element.querySelector(".captchaid").value}&captcha=${
      ...
      .then((text) => {
        ...
        element
          .querySelector(".message__action__container")
          .classList.remove("hidden");
...
const renderTourForm = async (element) => {
  const serviceElements = element
    .closest(".tourInventory__details__item")
    .querySelectorAll(".tourInventory__details__item__service");
  const sevice = [...serviceElements]
    .map((el) => {
      switch (parseInt(el.dataset.value)) {
        case 0:
          return "-";
        case 1654:
          return "O.R";
  ...
  if (document.querySelector(".execution__details__path__destinations")) {
    var form_end_time = document
      .querySelector(".execution__details__path__destinations")
      .querySelectorAll(".execution__details__path__item")[0]
      .querySelector(".__times__start").textContent;
  }
  $bc.setSource("db.tourForm", {
    hotelName: Array.from(
      element
        .closest(".tourInventory__details__item")
        .querySelectorAll(".showhotel")
    )
      .map((el) => el.textContent.trim())
      .join("، "),
    hotelRate: element
      .closest(".tourInventory__details__item")
      .querySelector(".tourInventory__details__item__rate").dataset.value,
    hotelService: sevice,
...
const renderTourInstallmentForm = async (element) => {
  ...
  $bc.setSource("db.tourFormInstallmentplan", {
    hotelName: element
      .closest(".tourInventory__details__item")
      .querySelector(".showhotel").textContent,
    hotelRate: element
      .closest(".tourInventory__details__item")
      .querySelector(".tourInventory__details__item__rate").dataset.value,
    hotelService: sevice,
  ...
  (FormhotelName = element
    ...
    (FormtourName = document.querySelector(
      ".tour__name__container"
    ).textContent),
    (FormdoubleP = element
      .closest(".tourInventory__details")
      .querySelectorAll(".tourInventory__details__item__double")[0]
...
const OnProcessedTourBookingForm = async (args) => {
  try {
    let dropdown = document.querySelector(
      ".tour__booking__form__modal__container .dropdown-modal-content"
    );
    if (dropdown) {
      dropdown.querySelector(".dropdown-modal-tour").classList.add("hidden");
    }
    ...
    if (errorid == "6") {
      if (page_lang === "fa") {
        ...
        document
          ...
          "اطلاعات شما با موفقیت ثبت شد. کارشناسان ما در اولین فرصت با شما تماس می‌گیرند";
      } else if (page_lang === "en") {
        document
          .querySelector(".tour__booking__form__modal__container")
          .querySelector(".message__action__container").innerHTML =
...
window.onload = function () {
  document
    .querySelectorAll(".tourItinerary__item__content")
    .forEach((content, index) => {
      content.classList.add("hidden");
      content
        .closest(".tourItinerary__item")
        .querySelector("svg")
        .classList.remove("rotate-180");
    });
  const firstItemContent = document.querySelector(
    ".tourItinerary__item__content"
  );
  if (firstItemContent) {
...
const toggleContentDay = async (element, parent, style, toggleDrop) => {
  try {
    if (toggleDrop) {
      ...
      listItem
        .querySelector(".tourItinerary__item__path svg")
...
const onrenderedSchmaTourBookingFormIns = async (args) => {
  try {
    const form = document.querySelector(
      ".tour__booking__form__modal__container_Ins .tour__booking__form__container"
    );
    ...
    const safeSet = (selector, prop, value) => {
      ...
      if (el && el[prop] !== undefined) {
        ...
        console.warn(
          `❌ المنت ${selector} پیدا نشد یا خاصیت ${prop} وجود ندارد.`
        );
      }
    };
    ...
    safeSet(
      ".hotel-name input",
      "value",
      form.querySelector(".hotel__name__container")?.textContent || ""
    );
    safeSet(
      ".hotel-service input",
      "value",
      form.querySelector(".hotel__service__container")?.textContent || ""
    );
    safeSet(
      ".hotel-rate input",
      "value",
      form.querySelector(".hotel__rate__container")?.textContent || ""
    );
    safeSet(
      ".tour-name input",
      "value",
      form.querySelector(".tour__name__container")?.textContent || ""
    );

    safeSet(
      ".total-amountF input",
      "value",
      modal.querySelector(".total-amount")?.innerText || ""
    );
    safeSet(
      ".total-advanceF input",
      "value",
      modal.querySelector(".Total-amount-facilities")?.innerText || ""
    );
    safeSet(
      ".amount-facilitiesF input",
      "value",
      modal.querySelector(".Total-advance-payment")?.innerText || ""
    );
    safeSet(
      ".amount-eachF input",
      "value",
      modal.querySelector(".amount-each-installment")?.innerText || ""
    );
  } catch (err) {
...
const OnProcessedTourBookingFormIns = async (args) => {
  try {
    let dropdown = document.querySelector(
      ".tour__booking__form__modal__container_Ins .dropdown-modal-content"
    );
    if (dropdown) {
      dropdown.querySelector(".dropdown-modal-tour").classList.add("hidden");
    }
