...
boxes.forEach((box) => {
  adjustTextColor(box);
  box.addEventListener("mouseover", () => adjustTextColor(box));
  box.addEventListener("mouseout", () => adjustTextColor(box));
});

const page_lang = document.querySelector("main").getAttribute("data-lang");
// execution - date inventory
const scrollToTourSection = (element, type) => {
  const targetSection = document.querySelector(`.${type}`);
  const navbar = document.querySelector(".navBar__container");
  const header = document.querySelector("header");

  if (!navbar || !targetSection) {
    console.error("Navbar or target section not found");
    return;
  }
  ...
  setTimeout(() => {
    ...
    const isNavbarFixed = navbarStyle.position === "fixed";
    const isHeaderFixed = headerStyle?.position === "fixed";

    let offset = 0;
    if (isNavbarFixed) {
      offset += navbar.offsetHeight;
      if (isHeaderFixed && header) {
...
const sectionsMap = [
  "tourExecution__container",
  "tourInventory__container",
  "tourItinerary__container",
  "tourDescription__container",
  "tourService__container",
  "tourEquipment__container",
  "tourFacility__container",
  "tourGallery__container",
  "tourComment__container",
  "tourRelated__container",
];
...
window.addEventListener("scroll", () => {
  ...
  sectionsMap.forEach((className, index) => {
    const el = document.querySelector(`.${className}`);
    if (el) {
      const sectionTop = el.offsetTop - offset - 10;
...
const callbackSourceInventoryView = async (args) => {
  try {
    ...
    console.error(
      "callbackSourceInventoryView=" + err.lineNumber + "," + err.message
    );
  }
};
...
const onrenderedExecutionDestinations = async () => {
  try {
    const container = document.querySelector(
      ".tourExecution__container__destinations"
    );
    if (!container) return;
    ...
    if (firstPathItem) {
      const cityEl = container.querySelector(".destinations__city");
      const detailsCityEl = firstPathItem.querySelector(".details__city");
      if (cityEl && detailsCityEl) {
        cityEl.textContent = detailsCityEl.textContent;
      }
    ...
    console.error(
      "onrenderedExecutionDestinations=" +
      (err.lineNumber || "?") +
      "," +
      err.message
    );
...
<g transform="translate(0.000000,1280.000000) scale(0.100000,-0.100000)" stroke="none" fill="${document.querySelector(".layout__body__container").dataset
            .primarycolor
          }">
...
<g transform="translate(0.000000,910.000000) scale(0.100000,-0.100000)" fill="${document.querySelector(".layout__body__container").dataset
            .primarycolor
          }" stroke="none">
...
<g transform="translate(0.000000,1280.000000) scale(0.100000,-0.100000)" fill="${document.querySelector(".layout__body__container").dataset
            .primarycolor
          }" stroke="none">
...
const renderRouteStop = async (element) => {
  try {
    if (element && element.info.stop && element.info.stop !== "") {
      return `<div class="my-8 text-sm">${getStopLabel()}<span class="mr-1 ml-1">${element.info.stop
  ...
  try {
    if (element) {
      ...
      if (page_lang === "fa" || page_lang === "ar") {
        ...
        gDate = [parseInt(date[0]), parseInt(date[1]), parseInt(date[2])];
      }
    ...
    if (
      ...
      const newHtml = `
        <div class="w-full max-lg:mt-5">
            <a href="tel:${document.querySelector(".company__info__tel").innerText
        }"
...
const renderTourDateModal = (element, className) => {
  try {
    const modal = document.querySelector(".tour__date__modal__container");
    modal.classList.remove(className);

    if (window.matchMedia("(max-width: 1024px)").matches) {
      modal.style.position = "fixed";
      modal.classList.add("isFixed");
    } else {
      modal.style.position = "sticky";
      modal.classList.remove("isFixed");
    }
  } catch (err) {
...
const renderHotels = async (element, type) => {
  try {
    if (element) {
      // === تنظیمات قابل تغییر ===
      const IMG_LOOKUP_URL = "/tour/basiscore/img/ver.1"; // سرویس/صفحه‌ای که با basiscoreid عکس می‌دهد (GET)
      const REQUEST_TIMEOUT_MS = 3000;

      // کش برای جلوگیری از درخواست تکراری
      const imageSrcCache = new Map();

      // هلپر: با basiscoreid عکس را Resolve می‌کند (GET با کوئری‌استرینگ)
      const resolveHotelImageSrc = async (basiscoreId, fallbackSrc) => {
        try {
          if (!basiscoreId) return fallbackSrc;
          const key = String(basiscoreId).trim();
          if (imageSrcCache.has(key)) return imageSrcCache.get(key);

          const controller = new AbortController();
          const to = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);

          const url = `${IMG_LOOKUP_URL}?basiscoreid=${encodeURIComponent(key)}`;
          const res = await fetch(url, { method: "GET", signal: controller.signal });
          clearTimeout(to);

          if (!res.ok) throw new Error(`HTTP ${res.status}`);

          // تلاش برای JSON سپس متن
          let out;
          const text = await res.text();
          try {
            const json = JSON.parse(text);
            out = json?.src || json?.url || json?.image || "";
          } catch {
            out = text; // متن ساده URL
          }

          if (typeof out === "string" && out.trim().length > 0) {
            const finalUrl = out.trim();
            imageSrcCache.set(key, finalUrl);
            return finalUrl;
          }
        } catch (e) {
          // console.warn("resolveHotelImageSrc error:", e);
        }
        return fallbackSrc;
      };

      let output = `<div class="md:flex  items-center justify-between mb-5"><div class="md:w-9/12  md:border-l md:border-dashed md:pl-4 md:ml-4 border-module-gray-2">`;

      const hotels = element.hotelinfo[0].hotels;

      for (const [index, item] of hotels.entries()) {
        // پیش‌فرض تصویر
        let defaultImg = `/common/images/img-symbol-sign.jpg`;
        const cont = document.querySelector(".layout__body__container");
        if (cont?.dataset?.noimgsign?.length > 0) {
          defaultImg = `/images/${cont.dataset.noimgsign}`;
        }

        // Checks
        const hotelIdRaw = item?.hotel?.hotelname1?.hotelid;
        const hasHotelId =
          hotelIdRaw !== undefined &&
          hotelIdRaw !== null &&
          String(hotelIdRaw).trim() !== "";

        const basiscoreIdRaw = item?.hotel?.basiscoreid;
        const hasValidBasiscoreId =
          basiscoreIdRaw !== undefined &&
          basiscoreIdRaw !== null &&
          String(basiscoreIdRaw).trim() !== "" &&
          String(basiscoreIdRaw).trim() !== "-";

        // اگر basiscoreid معتبر داشت، از سرویس عکس بگیر (GET)
        const resolvedImgSrc = hasValidBasiscoreId
          ? await resolveHotelImageSrc(basiscoreIdRaw, defaultImg)
          : defaultImg;

        // تگ img با src جدید/پیش‌فرض
        const imgTag = `
        <img src="/${resolvedImgSrc}"
             class="rounded-xl tourInventory__details__item__img transition-all hover:scale-105"
             data-pageName="${cont?.dataset?.pagenameinventory || ""}"
             data-id="${hasHotelId ? hotelIdRaw : ""}"
             data-basiscoreid="${hasValidBasiscoreId ? basiscoreIdRaw : ""}" 
             width="165" height="165" alt="" />`;

        // اگر hotelid نداشت و basiscoreid معتبر بود، img را لینک کن
        const imgWrapped = (!hasHotelId && hasValidBasiscoreId)
          ? `<a class="block" target="_blank" rel="noopener" href="/article.bc?id=${basiscoreIdRaw}">${imgTag}</a>`
          : imgTag;

        let borderClass =
          index === hotels.length - 1
            ? ""
            : "border-b border-dashed border-module-gray-2 mb-4 pb-4";

        output += `
           <div class="tourInventory__details__item__info flex gap-3 max-sm:flex-col ${borderClass}" data-index="${index}">
    <figure class="shrink-0 relative rounded-xl max-sm:mx-auto overflow-hidden">
        ${imgWrapped}
        ${item.hotel.label
          ? `
        <div class="absolute top-2 left-2 bg-white px-3 py-1 rounded-full text-xs font-medium shadow-lg backdrop-blur-sm bg-opacity-90">
            ${item.hotel.label}
        </div>`
          : ""
        }
    ...
    <figcaption class="md:w-9/12">
        <div class="md:flex justify-between max-md:text-center gap-8">
            <div class="md:w-3/5 text-module-gray-6">
                <div class="tourInventory__details__item__rate  max-md:justify-center flex gap-0.5" data-value="${item.hotel.star == "" ? 0 : item.hotel.star}">
                      <h3 class="text-lg lg:mb-4 max-lg:mb-5 hover:text-gray17 showhotel">${item.hotel.hotelname}</h3>
                </div>
                ${item.hotel.location
                  ? `
                <div class="mt-2">
                    <a href="${item.hotel.location
                      .replace(/"/g, '"')
                      .replace(/&/g, "&")}" 
                       target="_blank" 
                ...
                ${item.hotel.description
                  ? `<div class="text-sm mt-2">${item.hotel.description}</div>`
                  : ``
                }
                ${await renderHotelBooking(item.hotel.booking)}
            </div>

            <div class="md:w-2/5 md:ml-auto md:text-left">
                <div class="mb-4 md:flex md:justify-end">${await renderHotelRate(item.hotel)}</div>
                <div class="tourInventory__details__item__service md:justify-end max-md:justify-center flex gap-2 text-sm max-lg:mb-2" data-value="${item.hotel.service.vid}">
                    ${await renderServiceHotel(item.hotel)}
                </div>
            </div>
        </div>
    </figcaption>
...
<div class="md:w-3/12 flex gap-2">
    <div class="w-full max-lg:mt-5">
        <a href="tel:${document.querySelector(" .company__info__tel").innerText}"
            class="bg-white text-primary border border-solid border-primary rounded-md text-sm py-3 block text-center md:w-full mb-3 hover:bg-primary hover:text-white">
            ${page_lang === "fa"
        ...
        <button type="button" onclick="renderTourForm(this)"
            ...
            <span class="bg-primary py-3 px-3 rounded-md absolute top-0 right-0">
                ${page_lang === "fa"
                  ? "رزرو آنلاین"
                  : page_lang === "en"
                    ? "Online Booking"
  ...
  try {
    if (element) {
      ...
      for (const key of [
        ...
        if (!element.priceinfo[key] || element.priceinfo[key].length === 0) {
        }
      }

      let output = "";

      if (type == "doublecost" && element.priceinfo.doublecost.length > 0) {
        ...
        type == "singlecost" &&
        element.priceinfo.singlecost.length > 0
      ) {
        ...
        type == "triplecost" &&
        element.priceinfo.triplecost.length > 0
      ) {
        for (const item of element.priceinfo.triplecost) {
          output += `<div class="tourInventory__details__item__triple text-module-gray-4">
            <span class="tourInventory__details__item__price sm:text-xl font-bold">${new Intl.NumberFormat().format(
            item.triplecost.triplecostf
          )}</span>
            ${item.triplecost.tripleunit.length == 0
              ? ``
              : `<span class="tourInventory__details__item__unit sm:text-base max-sm:text-sm mr-1">${item.triplecost.tripleunit}</span>`
            }
          </div>`;
        }
        return output;
        ...
        type == "childwithbed" &&
        element.priceinfo.childwithbed.length > 0
      ) {
        ...
        element.priceinfo.childwithoutbed.length > 0
      ) {
      ...
      // اگر type معتبر نبود یا آرایه خالی بود
    }
  } catch (err) {
        ...
        document
          ...
          .forEach((e) => {
            ...
            if (e.dataset.id) {
              ...
              e.dataset.basiscoreid &&
              e.dataset.basiscoreid.trim() !== ""
            ) {
              const figure = e.closest("figure");
              if (figure) {
                const htmlImg = figure.innerHTML;
...
const onsubmitTourForm = async (element, event) => {
  try {
    ...
    fetch(`${element.getAttribute("action")}`, {
      ...
      body: `usedforid=${element.querySelector(".usedforid").value}&mid=${element.querySelector(".mid").value
        }&title=${element.querySelector(".title").value}&comment=${element.querySelector(".comment").value
        }&captchaid=${element.querySelector(".captchaid").value}&captcha=${element.querySelector(".captcha").value
        }&jsoninfo={"phone":"${element.querySelector(".phone").value}"}`,
    })
      ...
      .then((text) => {
        ...
        setTimeout(function () {
          ...
          element.querySelector(".title").value = "";
          element.querySelector(".phone").value = "";
          element.querySelector(".captcha").value = "";
          element.querySelector(".comment ").value = "";
        }, 3000);
      })
...
const renderTourForm = async (element) => {
  ...
  const itemRoot =
    element.closest(".tourInventory__details__item") ||
    element.closest(".tourInventory__details");
  if (!itemRoot) {
    console.warn("renderTourForm: item root not found");
    
    const tourName =
      document.querySelector("h1.tour__name__container")?.textContent.trim() || "-";
  ...
  const serviceMap = {
    ...
    1658: "ALL",
    1659: "U.ALL",
    1660: "Maximum All Inclusive",
  };

  const firstInfo = itemRoot.querySelector(
    ".tourInventory__details__item__info"
  );
  let doubleP = "-",
    singleP = "-",
    wBedP = "-",
  ...
  if (firstInfo) {
    const pick = (selPrice, selUnit) => {
      const priceEl = itemRoot.querySelector(selPrice);
      const unitEl = itemRoot.querySelector(selUnit);
      return [
        priceEl ? priceEl.textContent.trim() : "-",
        unitEl ? unitEl.textContent.trim() : "-",
      ];
    };

    [doubleP, doubleU] = pick(
  ...
  const hotels = [];
  itemRoot
    .querySelectorAll(".tourInventory__details__item__info[data-index]")
    .forEach((info) => {
      const hotelName =
        info.querySelector(".showhotel")?.textContent.trim() || "-";
      const hotelRate =
        info.querySelector(".tourInventory__details__item__rate")?.dataset
          ?.value || "-";
      const serviceEl = info.querySelector(
        ".tourInventory__details__item__service"
      );
      const serviceCode = serviceEl?.dataset
        ? parseInt(serviceEl.dataset.value)
        : 0;
      const hotelService = serviceMap[serviceCode] ?? "-";
  ...
  const tourName =
    document.querySelector(".tour__name__container")?.textContent.trim() || "-";
...
const onrenderedSchemaTourBookingForm = async (args) => {
  try {
    ...
    console.error("onrenderedSchemaTourBookingForm=" + err.message);
  }
};
const setValue = (selectorInput, selectorText) => {
  const container = document.querySelector(
    ".tour__booking__form__modal__container"
  );
  if (!container) return;
  const inputEl = container
    .querySelector(selectorInput)
    ?.querySelector("input");
  const textEls = container.querySelectorAll(selectorText);
  if (inputEl && textEls.length > 0) {
    const textValues = Array.from(textEls)
      .map((textEl) => textEl.textContent.trim())
      .join(", ");
    inputEl.value = textValues;
  }
};
...
document.addEventListener("DOMContentLoaded", function () {
  const firstParagraph = document.querySelector(".tour-desc-inner");
  const readMoreDesc = document.querySelector(".read-more-desc");
  if (firstParagraph && firstParagraph.offsetHeight > 48) {
    readMoreDesc.classList.remove("hidden");
  }
...
function checkToggleContent(
  ...
  if (!container) {
    console.error("checkToggleContent: container not found");
    return;
  }
  const content = container.querySelector(".toggle__content");
  if (!content) {
    console.error("checkToggleContent: content not found");
    return;
  }
...
const toggleContent = (element, parentSelector, style) => {
  try {
    const container = element.closest(parentSelector);
    if (!container) return console.error("toggleContent: container not found");

    const content = container.querySelector(".toggle__content");
    if (!content) return console.error("toggleContent: content not found");

    // اگر parent origins یا destinations بود، فقط hidden را toggle کن
    const pSel = String(parentSelector || "");
    const hiddenOnly = /tourExecution__container__(origins|destinations)/.test(
      pSel
    );
    if (hiddenOnly) {
    ...
    // در غیر این صورت: منطق collapse (max-height + overflow)
    let toggleTextEl = element;
    if (["svg", "path"].includes(element.tagName?.toLowerCase()))
      toggleTextEl = container.querySelector(
        ".toggle__text, .tourItinerary__item__more__link"
      );
    ...
    const isCollapsed =
      currentMax !== "none" && currentMax !== "" && currentMax !== "0px";
    ...
    if (isCollapsed) {
      ...
      if (toggleTextEl) {
        const lang = (
          typeof page_lang === "string" ? page_lang : "en"
        ).toLowerCase();
    ...
    console.error("toggleContent=", err);
  }
};

document.addEventListener("DOMContentLoaded", () => {
  document
    .querySelectorAll(".tourDescription__container")
    .forEach((container) => {
      const btn = container.querySelector("span.text-primary");
      if (btn) checkToggleContent(btn, ".tourDescription__container", 3, 24);
    });
});
...
if (document.querySelectorAll(".tourItinerary__item")[0]) {
  ...
  if (
    document
      .querySelectorAll(".tourItinerary__item")[0]
      .querySelector(".tourItinerary__path")
  ) {
    document
      .querySelectorAll(".tourItinerary__item")[0]
      .querySelector(
        ".tourItinerary__path"
      ).innerHTML = `<svg class="relative z-10" width="18" height="45" viewBox="0 0 18 45" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
        ...
        fill="${document.querySelector(".layout__body__container").dataset
        .primarycolor
      }" />
</svg>`;
  }
  ...
  if (document.querySelectorAll(".tourItinerary__end__path")[0]) {
    document
      .querySelectorAll(".tourItinerary__item")
    [
      document.querySelectorAll(".tourItinerary__item").length - 1
    ].querySelector(
      ".tourItinerary__end__path"
...
if (document.querySelectorAll(".tourItinerary__item__largetext")[0]) {
  document.querySelectorAll(".tourItinerary__item__largetext").forEach((e) => {
    if (e.textContent.length > 71) {
      e.closest(".tourItinerary__item__path")
        .querySelector(".tourItinerary__item__more__link")
        .classList.remove("hidden");
    }
  });
}
...
document.querySelectorAll(".tourview-cost").forEach(function (element) {
...
window.addEventListener("scroll", function () {
  if (window.innerWidth > 1024) {
    let element = document.querySelector(".navBar__container");
    let scrollPosition = window.scrollY;
    if (scrollPosition > 200) {
      element.classList.add("navBar__fixed");
    } else {
      element.classList.remove("navBar__fixed");
    }
  }
});
const onrenderedTourBookingForm = async () => {
  try {
    const lang = (
      typeof page_lang === "string" ? page_lang : "en"
    ).toLowerCase();
    const starLabel = lang === "fa" ? "ستاره" : lang === "ar" ? "نجمة" : "Star";

    // Get the data from the tour__hotel__booked__container
    const hotelData = JSON.parse(
      document
        .querySelector(".tour__hotel__booked__container")
        .getAttribute("data-info")
    );

    // Iterate through each hotel and add the corresponding HTML
    hotelData.forEach((hotel) => {
      const hotelHTML = `
                <div class="flex mb-10">
                    <svg class="ml-3" width="30" height="30" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24" stroke-width="1.5" stroke="#000">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m-.75 4.5H21m-3.75 3.75h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Z" />
                    </svg>
                    <div>
                        <h3 class="hotel__name__container mb-1">${hotel.hotelName}</h3>
                        <div><span class="ml-2">
                            <svg class="inline-block ml-1" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M17.5999 14.79C17.5409 14.434 17.6599 14.071 17.9189 13.82L21.6589 10.28C21.9669 9.991 22.0799 9.551 21.9489 9.15C21.8099 8.75 21.4589 8.46 21.0389 8.4L16.0689 7.679C15.7099 7.624 15.3989 7.397 15.2389 7.07L13.0199 2.6C12.8469 2.264 12.5159 2.039 12.1399 2H11.7189L11.5489 2.07L11.4399 2.11C11.3799 2.145 11.3259 2.188 11.2789 2.24L11.1889 2.31C11.1079 2.388 11.0409 2.48 10.9889 2.58L8.79888 7.07C8.62888 7.41 8.29688 7.64 7.91888 7.679L2.94888 8.4C2.53588 8.465 2.19388 8.754 2.05988 9.15C1.92188 9.547 2.02688 9.987 2.32888 10.28L5.93988 13.78C6.19888 14.035 6.31788 14.4 6.25888 14.759L5.36888 19.679C5.27188 20.274 5.66688 20.838 6.25888 20.95C6.50188 20.989 6.74988 20.95 6.96888 20.84L11.3989 18.519C11.4829 18.473 11.5749 18.443 11.6689 18.429H11.9399C12.1149 18.434 12.2859 18.478 12.4399 18.56L16.8689 20.87C17.2419 21.07 17.6969 21.04 18.0389 20.79C18.3879 20.549 18.5639 20.127 18.4889 19.71L17.5999 14.79Z" fill="#FFBF1C"></path>
                            </svg><span class="ml-1 hotel__rate__container">${hotel.hotelRate}</span> ${starLabel}</span>
                            (<span class="hotel__service__container">${hotel.hotelService}</span>)
                        </div>
                    </div>
                </div>
            `;

      // Insert the hotel HTML into the div
      document
        .querySelector(".tour__hotel__booked__container")
        .insertAdjacentHTML("beforeend", hotelHTML);
    });
    if(document.querySelectorAll('.hotel__name__container').length){
        setValue(".hotel-name", ".hotel__name__container");
    }else{
       setValue(".hotel-name", ".tour__name__container");
    }
    setValue(".hotel-service", ".hotel__service__container");
    setValue(".hotel-rate", ".hotel__rate__container");
  } catch (err) {
    console.error("onrenderedTourBookingForm=" + err.message);
  }
};
